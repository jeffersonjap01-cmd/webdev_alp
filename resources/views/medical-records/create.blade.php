        ...
        <div class="px-4 py-5 sm:p-6">
            <form action="{{ route('medical-records.store') }}" method="POST" class="space-y-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label for="appointment_id" class="block text-sm font-medium text-gray-700">Janji Temu (Opsional)</label>
                        <select name="appointment_id" id="appointment_id" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md @error('appointment_id') border-red-300 @enderror">
                        ...
                        @error('appointment_id')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                        <p class="mt-1 text-sm text-gray-500">Opsional. Pilih janji temu terkait jika ada.</p>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="symptoms" id="symptoms" rows="3" 
                                  ...
                                  placeholder="Deskripsikan gejala yang diamati">{{ old('symptoms') }}</textarea>
                        @error('symptoms')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <!-- Cost -->
                    <div>
                        <label for="cost" class="block text-sm font-medium text-gray-700">Biaya (Rp)</label>
                        <input type="number" name="cost" id="cost" min="0" step="1000" 
                               value="{{ old('cost') }}" 
                               class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('cost') border-red-300 @enderror" 
