...
@section('title', 'Customer Details - VetCare')
        ...
        <div class="md:flex md:items-center md:justify-between">
            <div class="flex-1 min-w-0">
                <div class="flex items-center">
                    <img class="h-16 w-16 rounded-full" src="https://ui-avatars.com/api/?name={{ urlencode($customer->name) }}&color=7F9CF5&background=EBF4FF" alt="{{ $customer->name }}">
                    <div class="ml-4">
                        ...
                        <div class="mt-1 flex flex-col sm:flex-row sm:flex-wrap sm:mt-0 sm:space-x-6">
                            ...
                            <div class="mt-2 flex items-center text-sm text-gray-500">
                                ...
                                Bergabung: {{ $customer->registered_date ? $customer->registered_date->format('d M Y') : 'Unknown' }}
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="mt-4 flex md:mt-0 md:ml-4">
                <a href="{{ route('customers') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                    <i class="fas fa-arrow-left mr-2"></i>
                    Kembali
                </a>
                @if(auth()->user()->role === 'admin')
                <a href="{{ route('customers.edit', $customer) }}" class="ml-3 inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                ...
                <div class="px-4 py-5 sm:p-6">
                    ...
                    <dl class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <dd class="mt-1 text-sm text-gray-900">{{ $customer->pets()->count() }}</dd>
                        </div>
                        @if($customer->address)
                        ...
                        <div>
                            ...
                            <dd class="mt-1 text-sm text-gray-900">{{ $customer->registered_date ? $customer->registered_date->format('d M Y') : 'Unknown' }}</dd>
                        </div>
                        <div>
            ...
            <div class="bg-white shadow rounded-lg">
                <div class="px-4 py-5 sm:p-6">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <a href="{{ route('pets.index', ['owner' => $customer->user_id ?? $customer->id]) }}" class="text-sm text-blue-600 hover:text-blue-500">
                            Lihat Semua
                        </a>
                    </div>
                    
                    @if($customer->pets && $customer->pets->count() > 0)
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        @foreach($customer->pets->take(4) as $pet)
                    ...
                    @if($customer->pets->count() > 4)
                    <div class="mt-4 text-center">
                        <p class="text-sm text-gray-500">Dan {{ $customer->pets->count() - 4 }} hewan peliharaan lainnya</p>
                    </div>
                    @endif
                    @else
                    ...
                    @if($customer->pets)
                        @php
                            $recentAppointments = \App\Models\Appointment::with(['pet', 'doctor'])
                                ->whereHas('pet', function($q) use ($customer) {
                                    $q->where('user_id', $customer->user_id ?? $customer->id);
                                })
                                ->latest()
                                ->take(5)
                                ->get();
        ...
        <div class="space-y-6">
                ...
                <div class="px-4 py-5 sm:p-6">
                    ...
                    <dl class="space-y-4">
                        <div class="flex justify-between">
                            ...
                            <dd class="text-sm text-gray-900">{{ $customer->pets()->count() }}</dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="text-sm font-medium text-gray-500">Total Janji Temu</dt>
                            <dd class="text-sm text-gray-900">
                                @if($customer->pets)
                                    {{ \App\Models\Appointment::whereHas('pet', function($q) use ($customer) {
                                        $q->where('user_id', $customer->user_id ?? $customer->id);
                                    })->count() }}
                                @else
                                    0
            ...
            <div class="bg-white shadow rounded-lg">
                <div class="px-4 py-5 sm:p-6">
                    ...
                    <div class="space-y-3">
                        @if($customer->phone)
                        <a href="tel:{{ $customer->phone }}" class="w-full inline-flex justify-center items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
                            <i class="fas fa-phone mr-2"></i>
                            Telepon
                        </a>
                        @endif
                        <a href="mailto:{{ $customer->email }}" class="w-full inline-flex justify-center items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
                            <i class="fas fa-envelope mr-2"></i>
                            Email
                        </a>
                        @if($customer->address)
                        <a href="https://maps.google.com/?q={{ urlencode($customer->address) }}" target="_blank" class="w-full inline-flex justify-center items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
