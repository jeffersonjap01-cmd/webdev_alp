        ...
        <div class="space-y-6">
            ...
            <div class="bg-white shadow rounded-lg">
                <div class="px-4 py-5 sm:p-6">
                    ...
                    <dl class="space-y-4">
                        <div class="flex items-center">
                            <img class="h-10 w-10 rounded-full" src="https://ui-avatars.com/api/?name={{ urlencode($prescription->pet->owner->name ?? 'Unknown') }}&color=7F9CF5&background=EBF4FF" alt="{{ $prescription->pet->owner->name ?? 'Unknown' }}">
                            <div class="ml-3">
                                <dt class="text-sm font-medium text-gray-900">{{ $prescription->pet->owner->name ?? 'Unknown' }}</dt>
                                <dd class="text-sm text-gray-500">{{ $prescription->pet->owner->email ?? 'Unknown' }}</dd>
                            </div>
                        </div>
                        @if($prescription->pet->owner && $prescription->pet->owner->phone)
                        <div>
                            <dt class="text-sm font-medium text-gray-500">Telepon</dt>
                            <dd class="mt-1 text-sm text-gray-900">{{ $prescription->pet->owner->phone }}</dd>
                        </div>
                        @endif
                    </dl>
                    <div class="mt-4">
                        <a href="{{ route('owners.show', $prescription->pet->owner) }}" class="w-full inline-flex justify-center items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
